...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -trimpath -buildvcs=false -ldflags='-s -w -buildid=' -o /out/xray-entry ./cmd/xray-entry
...
RUN mkdir -p /out/usr/local/bin /out/usr/local/share/xray /out/etc/ssl/certs /out/tmp \
    && unzip -j /tmp/xray.zip xray geoip.dat geosite.dat -d /out/usr/local/share/xray \
    ...
    && chmod 0555 /out/usr/local/bin/xray \
    && chmod 0444 /out/usr/local/share/xray/geoip.dat /out/usr/local/share/xray/geosite.dat /out/etc/ssl/certs/ca-certificates.crt \
    && chmod 1777 /out/tmp

FROM scratch
COPY --from=xray-verified /out/ /
COPY --from=entry-build /out/xray-entry /usr/local/bin/xray-entry
USER 65532:65532
